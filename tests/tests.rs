...
fn test_scan() {
  ...
  assert_eq!(scan("1-1").unwrap(), vec![Noun(K::Bool(1)), KW::Verb { name: "-".to_string() }, Noun(K::Bool(1))]);
  assert_eq!(scan("0N-1").unwrap(), vec![Noun(K::Int(None)), KW::Verb { name: "-".to_string() }, Noun(K::Bool(1))]);
...
#[test]
fn test_scan_lambda() {
  let tokens = vec![KW::LCB, KW::Noun(K::Name("x".into())), KW::Verb { name: "+".to_string() }, KW::Noun(K::Name("y".into())), KW::RCB];
...
fn test_eval() {
  ...
  assert_eq!(scan("2 + 2").unwrap(), vec![Noun(K::Int(Some(2))), Verb { name: "+".to_string() }, Noun(K::Int(Some(2)))]);
  ...
  assert_eq!(scan("1.0 + 2.0").unwrap(), vec![Noun(K::Float(1.0)), Verb { name: "+".to_string() }, Noun(K::Float(2.0))]);
  ...
  assert_eq!(eval(&mut env, scan("1 0 1 0 1 + 2").unwrap()).unwrap(), Noun(K::IntArray(arr!([3, 2, 3, 2, 3]))));
...
fn test_parens() {
  ...
  assert_eq!(eval(&mut env, scan("((2 + 0 + 0 + 1 - 1) * (3*1)) + 1").unwrap()).unwrap(), Noun(K::Int(Some(7))));
}
...
fn test_array_maths() {
  ...
  assert_eq!(format!("{:?}", eval(&mut env, scan("3.14 + 1 2 3").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([4.14, 5.14, 6.14])))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("3.14 + 1.0 2.0 3.0").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([4.14, 5.14, 6.14])))));

  assert_eq!(format!("{:?}", eval(&mut env, scan("1.1 2.1 + 1").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([2.1, 3.1])))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("1.1 2.1 + 2").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([3.1, 4.1])))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("1.1 2.1 + 2.0").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([3.1, 4.1])))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("1.1 2.1 + 1 1").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([2.1, 3.1])))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("1.1 2.1 + 2 2").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([3.1, 4.1])))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("1.1 2.1 + 2.0 2.0").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([3.1, 4.1])))));

  assert_eq!(format!("{:?}", eval(&mut env, scan("1.1 2.1 - 1").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([0.1, 1.1])))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("1.1 2.1 - 2").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([-0.9, 0.1])))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("1.1 2.1 - 2.0").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([-0.9, 0.1])))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("1.1 2.1 - 1 1").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([0.1, 1.1])))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("1.1 2.1 - 2 2").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([-0.9, 0.1])))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("1.1 2.1 - 2.0 2.0").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([-0.9, 0.1])))));

  assert_eq!(format!("{:?}", eval(&mut env, scan("1.1 2.1 * 1").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([1.1, 2.1])))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("1.1 2.1 * 2").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([2.2, 4.2])))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("1.1 2.1 * 2.0").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([2.2, 4.2])))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("1.1 2.1 * 1 1").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([1.1, 2.1])))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("1.1 2.1 * 2 2").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([2.2, 4.2])))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("1.1 2.1 * 2.0 2.0").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([2.2, 4.2])))));

  assert_eq!(format!("{:?}", eval(&mut env, scan("1.1 2.1 % 1").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([1.1, 2.1])))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("1.1 2.1 % 2").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([0.55, 1.05])))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("1.1 2.1 % 2.0").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([0.55, 1.05])))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("1.1 2.1 % 1 1").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([1.1, 2.1])))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("1.1 2.1 % 2 2").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([0.55, 1.05])))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("1.1 2.1 % 2.0 2.0").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([0.55, 1.05])))));
}

#[test]
fn test_iota() {
  let mut env = Env { names: HashMap::new(), parent: None };
  assert_eq!(format!("{:?}", eval(&mut env, scan("! 4").unwrap()).unwrap()), format!("{:?}", Noun(K::IntArray(arr!([0, 1, 2, 3i64])))));
}

#[test]
fn test_strings() {
  let mut env = Env { names: HashMap::new(), parent: None };
  assert_eq!(format!("{:?}", eval(&mut env, scan("\"a\"").unwrap()).unwrap()), format!("{:?}", Noun(K::Char('a'))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("\"\"").unwrap()).unwrap()), format!("{:?}", Noun(K::CharArray(Series::new("", "")))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("\"abcABC\"").unwrap()).unwrap()), format!("{:?}", Noun(K::CharArray(Series::new("", "abcABC")))));
}

#[test]
fn test_symbols() {
  let mut env = Env { names: HashMap::new(), parent: None };
  assert_eq!(format!("{:?}", eval(&mut env, scan("`a").unwrap()).unwrap()), format!("{:?}", Noun(K::Symbol("a".to_string()))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("`abc ").unwrap()).unwrap()), format!("{:?}", Noun(K::Symbol("abc".to_string()))));
  assert_eq!(
...
fn test_lists() {
  ...
  assert_eq!(format!("{:?}", eval(&mut env, scan("(1+1;2+2)").unwrap()).unwrap()), format!("{:?}", Noun(K::IntArray(arr!([2, 4i64])))));

  assert_eq!(format!("{:?}", eval(&mut env, scan("(1;2;3)").unwrap()).unwrap()), format!("{:?}", Noun(K::IntArray(arr!([1, 2, 3i64])))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("(1;0;1)").unwrap()).unwrap()), format!("{:?}", Noun(K::BoolArray(arr!([1, 0, 1u8])))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("(1;0;1.5)").unwrap()).unwrap()), format!("{:?}", Noun(K::FloatArray(arr!([1.0, 0.0, 1.5f64])))));
  assert_eq!(format!("{:?}", eval(&mut env, scan("(\"a\";\"b\";\"c\")").unwrap()).unwrap()), format!("{:?}", Noun(K::CharArray(Series::new("", "abc")))));
...
fn test_list_maths() {
  ...
  assert_eq!(
    ...
    format!("{:?}", Noun(K::List(vec![K::IntArray(arr!([2, 3i64])), K::IntArray(arr!([4, 5i64]))])))
  );
}
...
fn test_dict() {
  ...
  assert_eq!(format!("{:?}", eval(&mut env, scan("`a`b!(\"a\";42)").unwrap()).unwrap()), format!("{:?}", KW::Noun(d1)));

  let k = K::SymbolArray(Series::new("a", ["a", "b"]).cast(&DataType::Categorical(None)).unwrap());
  let v = K::List(vec![K::Int(Some(1)), K::Int(Some(2))]);
  let d1 = v_makedict(k, v).unwrap();
  assert_eq!(format!("{:?}", eval(&mut env, scan("`a`b!1 2").unwrap()).unwrap()), format!("{:?}", KW::Noun(d1)));

  let k = K::SymbolArray(Series::new("a", ["a", "b"]).cast(&DataType::Categorical(None)).unwrap());
  let v = K::List(vec![K::Int(Some(1)), K::Int(None)]);
  let d1 = v_makedict(k, v).unwrap();
  assert_eq!(format!("{:?}", eval(&mut env, scan("`a`b!1 0N").unwrap()).unwrap()), format!("{:?}", KW::Noun(d1)));

  let k = K::SymbolArray(Series::new("a", ["a", "b"]).cast(&DataType::Categorical(None)).unwrap());
  let v = K::List(vec![K::Float(1.5), K::Float(2.5)]);
  let d1 = v_makedict(k, v).unwrap();
  assert_eq!(format!("{:?}", eval(&mut env, scan("`a`b!1.5 2.5").unwrap()).unwrap()), format!("{:?}", KW::Noun(d1)));

  let k = K::SymbolArray(Series::new("a", ["a", "b"]).cast(&DataType::Categorical(None)).unwrap());
  let v = K::List(vec![K::Char('a'), K::Char('b')]);
  let d1 = v_makedict(k, v).unwrap();
  assert_eq!(format!("{:?}", eval(&mut env, scan("`a`b!\"ab\"").unwrap()).unwrap()), format!("{:?}", KW::Noun(d1)));

  let k = K::SymbolArray(Series::new("a", ["a", "b"]).cast(&DataType::Categorical(None)).unwrap());
  let v = K::List(vec![K::Bool(1), K::Bool(0)]);
  let d1 = v_makedict(k, v).unwrap();
  assert_eq!(format!("{:?}", eval(&mut env, scan("`a`b!1 0").unwrap()).unwrap()), format!("{:?}", KW::Noun(d1)));
  ...
  let d1 = v_makedict(k, v).unwrap();
  assert_eq!(format!("{:?}", eval(&mut env, scan("`a`b`c!1").unwrap()).unwrap()), format!("{:?}", KW::Noun(d1)));
}
...
fn test_dict_maths() {
  ...
  let k = K::SymbolArray(Series::new("a", ["a", "b", "c", "d"]).cast(&DataType::Categorical(None)).unwrap());
...
fn test_table_reader() {
  let mut env = Env { names: HashMap::new(), parent: None };
  let mut df = DataFrame::new(vec![Series::new("a", [1, 2, 3i64]), Series::new("b", [4, 5, 6i64])]).unwrap();
...
fn test_parse_functions() {
  ...
  //
  let f =
    KW::Function { body: vec![KW::Noun(K::Int(Some(2))), KW::Verb { name: "*".to_string() }, KW::Noun(K::Name("x".to_string()))], args: vec!["x".to_string()] };
  let mut env = Env { names: HashMap::new(), parent: None };
